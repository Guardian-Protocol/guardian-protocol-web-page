...
class GearProgramStorage {
    ...
    async getProgramPages(programId, program, at) {
        const pages = {};
        const query = this._api.specVersion >= consts.SPEC_VERSION.V1100
            ? this._api.query.gearProgram.memoryPages
            : this._api.query.gearProgram.memoryPageStorage;
        const args = this._api.specVersion >= consts.SPEC_VERSION.V1100 ? [programId, program.memoryInfix] : [programId];
        for (const page of program.pagesWithData) {
            pages[page.toNumber()] = util.u8aToU8a(await this._api.provider.send('state_getStorage', [query.key(...args, page), at]));
        }
        return pages;
    }
    /**
